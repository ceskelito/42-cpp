...
<<<<<<< HEAD
	...
	// Find the boundElement in main chain (b5->a5, b4->a4 ...)
	for (boundElement = main.begin(); boundElement != main.end(); ++boundElement)
		if (boundElement->label == 'a' && boundElement->index == elementToInsert->index)
			break;

	while (boundElement != main.begin()) {

		previousElement = boundElement - 1;

		g_cmps++;
		if (previousElement->value <= elementToInsert->value)
			break;
